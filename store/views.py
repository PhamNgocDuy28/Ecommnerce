...
from .models import Category, ProductGallery, ReviewRating
...
def store(request, category_slug=None):
    ...
    if category_slug != None:
        ...
        products = Product.objects.filter(category=categories, is_validate=True)
        paginator = Paginator(products, 6)
        page = request.GET.get('page')
        paged_products = paginator.get_page(page)
        product_count = products.count()
    else:
        products = Product.objects.all().filter(is_validate=True).order_by('id')
        paginator = Paginator(products, 6)
        page = request.GET.get('page')
        paged_products = paginator.get_page(page)
        product_count = products.count()
